...
public class NettyServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        //ByteBuf 是 netty提供的，不是nio的bytebuffer ,本质是对bytebuffer的封装，减少了flip的读写切换
        ByteBuf byteBuf = (ByteBuf) msg;
        logger.info("收到客户端：" +ctx.channel().remoteAddress() + "发送的消息: " + byteBuf.toString(CharsetUtil.UTF_8));

        Channel channel = ctx.channel();
        channelGroup.forEach(index ->{
            if (channel != index ){
                index.writeAndFlush(byteBuf);
            }
        });
    }
